...
const IS_LOW_END_MOBILE = typeof navigator !== "undefined" && ((navigator.hardwareConcurrency || 4) <= 2 || (navigator.deviceMemory || 4) <= 2 || /Android/i.test(navigator.userAgent) && (navigator.hardwareConcurrency || 4) <= 4);
const PAGE_SIZE = 15;
const MAX_DOM_POSTS = (() => {
...
const FALLBACK_COOLDOWN_MS = 6e4;
const RECENT_6H = 6 * 60 * 60 * 1e3;
const RECENT_24H = 24 * 60 * 60 * 1e3;
const RECENT_72H = 72 * 60 * 60 * 1e3;
const PREFETCH_AHEAD = 4;
const PREFETCH_IDLE_WAIT = 150;
const RESOLVE_CONCURRENCY = 6;
const PREFETCH_THRESHOLD = 0.7;
const URGENT_THRESHOLD = 0.9;
const SILENT_COOLDOWN_MS = 8e3;
const SCROLL_IDLE_MS = 2e3;
const EXPLORATION_RATE = 0.1;
const PREFETCH_URL_MAX = 300;
const SCORE_CACHE = /* @__PURE__ */ new Map();
const SCORE_CACHE_VERSION = { v: 0 };
const SCORE_CACHE_MAX = 3e3;
...
const LIVE_POOL_KEY = "feed_live_pool_v17";
const SEEN_IDS_KEY = "feed_seen_ids_v17";
...
const WEIGHTS_KEY = "feed_score_weights_v17";
const loadWeights = () => {
  try {
    ...
    const total = Object.values(w).reduce((a, b) => a + b, 0);
    if (total > 0.5 && total < 2) return normalizeWeights({ ...DEFAULT_SCORE_WEIGHTS, ...w });
  } catch {
  }
...
const VEC_CACHE = /* @__PURE__ */ new Map();
const VEC_CACHE_MAX = 2e3;
...
const scorePost = (post, now = Date.now()) => {
  ...
  score += Math.min(15, engagementRate * 1e3);
...
const loadBehavior = () => {
  try {
    ...
    return { authorBoosts: {}, categoryBoosts: {}, typeBoosts: {}, postSignals: {}, ts: Date.now() };
  }
};
const saveBehavior = (behavior) => {
  try {
    localStorage.setItem(BEHAVIOR_KEY, JSON.stringify({ ...behavior, ts: Date.now() }));
  } catch {
  }
};
const POSITION_PROPENSITY = [1, 0.85, 0.72, 0.61, 0.52, 0.44, 0.38, 0.33, 0.28, 0.25];
...
const createBehaviorTracker = () => {
  ...
  const scheduleSave = () => {
    ...
    saveTimer = setTimeout(flush, 2e3);
    dirty = true;
  };
  ...
  return {
    ...
    onDwell: (post, ms) => {
      if (ms < 2e3) return;
      const boost = ms > 1e4 ? 6 : ms > 5e3 ? 3 : 1;
...
const createUserEmbedding = () => {
  ...
  const load = () => {
    try {
      ...
      if (raw) vec = new Float32Array(JSON.parse(raw));
    } catch {
    }
  };
  const save = () => {
    try {
      localStorage.setItem(EMB_KEY, JSON.stringify(Array.from(vec)));
    } catch {
    }
  };
  load();
...
const MULTI_OBJ_WEIGHTS = { like: 0.25, comment: 0.35, share: 0.3, watch: 0.2, hide: -0.4 };
const multiObjectiveScore = (post, userEmbedding, baseScore) => {
  ...
  const score = pLike * MULTI_OBJ_WEIGHTS.like + pComment * MULTI_OBJ_WEIGHTS.comment + pShare * MULTI_OBJ_WEIGHTS.share + pWatch * MULTI_OBJ_WEIGHTS.watch + pHide * MULTI_OBJ_WEIGHTS.hide;
  return Math.max(0, Math.min(100, score * 100));
};
const velocityScore = (post, now = Date.now()) => {
  const ageH = Math.max((now - new Date(post.createdAt || 0).getTime()) / 36e5, 0.1);
  const eng = (post.likesCount || 0) + (post.commentsCount || 0) * 3 + (post.sharesCount || 0) * 5;
  return Math.min(100, Math.log1p(eng / ageH) * 12);
};
const socialGraphBoost = (post, followingIds = /* @__PURE__ */ new Set()) => {
...
const LTM_TTL = 30 * 24 * 60 * 60 * 1e3;
const LTM_DECAY_HALF = 7 * 24 * 60 * 60 * 1e3;
...
const createLongTermMemory = () => {
  ...
  const scheduleSave = () => {
    dirty = true;
    clearTimeout(saveTimer);
    saveTimer = setTimeout(flush, 3e3);
  };
  const flush = async () => {
...
const contentQualityScore = (post) => {
  ...
  if (len > 100 && len < 1e3) score += 15;
  else if (len >= 1e3) score += 8;
...
const TREND_WINDOW_MS = 2 * 60 * 60 * 1e3;
...
const EMOTION_LEXICON = {
  joy: { words: ["heureux", "joie", "bonheur", "super", "g\xE9nial", "excellent", "parfait", "bravo", "magnifique", "happy", "joy", "wonderful", "amazing", "great", "love", "beautiful"], score: 1.2 },
  interest: { words: ["int\xE9ressant", "fascinant", "d\xE9couverte", "apprentissage", "interesting", "fascinating", "learn", "discover", "knowledge"], score: 1.1 },
  surprise: { words: ["surprenant", "inattendu", "r\xE9v\xE9lation", "wow", "surprising", "unexpected", "reveal"], score: 1.05 },
  anger: { words: ["honte", "scandale", "r\xE9voltant", "inacceptable", "horrible", "d\xE9gueulasse", "shame", "scandal", "outrage", "disgusting", "unacceptable", "hate"], score: -1.5 },
  fear: { words: ["danger", "peur", "alerte", "menace", "effrayant", "catastrophe", "fear", "alert", "threat", "scary", "disaster"], score: -0.8 },
  disgust: { words: ["d\xE9go\xFBt", "r\xE9pugnant", "ignoble", "abject", "disgusting", "repulsive", "revolting"], score: -1.2 },
  ragebait: { words: ["complot", "woke", "dictature", "ils veulent", "on vous cache", "moutons", "sheeple", "conspiracy", "they want", "hidden truth"], score: -2 }
};
...
const SESSION_KEY = "feed_session_v17";
const SESSION_INACTIVITY = 30 * 60 * 1e3;
...
const createSessionTracker = () => {
  ...
  const persist = () => {
    try {
      sessionStorage.setItem(SESSION_KEY, JSON.stringify({ sessionStart, postsViewed, lastActivity }));
    } catch {
    }
  };
  try {
  ...
  return {
    ...
    getMultiplier(post) {
      checkReset();
      const mins = (Date.now() - sessionStart) / 6e4;
...
const createCausalCalibrator = () => {
  ...
  try {
    ...
    if (raw) {
      ...
      if (Date.now() - p.ts < 7 * 24 * 60 * 60 * 1e3) data = p;
    }
  } catch {
  }
  const save = () => {
    try {
      localStorage.setItem(CAUSAL_KEY, JSON.stringify(data));
    } catch {
    }
  };
  return {
    ...
    recordShown: (isControl) => {
      isControl ? data.control.shown++ : data.treatment.shown++;
      if (data.control.shown % 20 === 0) save();
    },
    recordInteraction: (isControl) => {
      isControl ? data.control.interacted++ : data.treatment.interacted++;
      save();
    },
    getLift: () => {
    ...
    recalibrateWeights: (weights, onSave) => {
      const lift = data.control.shown >= CAUSAL_MIN_SAMPLES && data.treatment.shown >= CAUSAL_MIN_SAMPLES ? (() => {
        const cC = data.control.interacted / data.control.shown, cT = data.treatment.interacted / data.treatment.shown;
        return cC > 0 ? (cT - cC) / cC : null;
      })() : null;
      ...
      if (lift < -0.05) {
        Object.keys(updated).forEach((k) => {
          if (k !== "base") updated[k] *= 0.95;
        });
        updated.base = Math.min(0.3, updated.base * 1.1);
      } else if (lift > 0.1) {
        Object.keys(updated).forEach((k) => {
          if (k !== "base") updated[k] = Math.min(0.4, updated[k] * 1.02);
        });
      }
...
const applyMMRReranking = (scoredPosts) => {
  ...
  const withVecs = scoredPosts.map((item) => ({ ...item, vec: postToVec(item.post) }));
  const result = [], remaining = [...withVecs];
...
const applyExplorationBudget = (rankedPosts, totalSlots) => {
  ...
  explore.forEach((p, i) => {
    ...
    result.splice(at, 0, { ...p, _isExplore: true });
  });
  return result;
};
const scoreOnePostRaw = (p, ctx, seenIds, followingIds, now) => {
  ...
  const seenDecay = seenIds.has(p._id || p._displayKey) ? 0.5 : 1;
  const isControl = calib.isControlGroup(p._id);
  calib.recordShown(isControl);
  const rawScore = isControl ? Math.random() * 50 + 10 : behavioral * weights.behavioral + mobjective * weights.multiObj + velocity * weights.velocity + social * weights.social + base * weights.base + ctxTime * weights.contextTime + ltmScore * weights.longTermMem + quality * weights.quality + trendAmp * weights.trendAmp + fatigueP + emotionP;
...
const smartBuildFeed = (posts, bots, seenIds, followingIds = /* @__PURE__ */ new Set(), ctx = null) => {
  ...
  const scoredBots = bots.map((p) => {
    ...
    const total = (base * 0.5 + velocity * 0.3) * (seenIds.has(p._id || p._displayKey) ? 0.4 : 1);
...
const scheduleIdlePrefetch = (posts, fromIndex, count = PREFETCH_AHEAD, minScore = 0) => {
  ...
  if (typeof requestIdleCallback !== "undefined") requestIdleCallback(run, { timeout: 2e3 });
  else setTimeout(run, PREFETCH_IDLE_WAIT);
};
const URL_CACHE_TTL = 80 * 60 * 1e3;
const URL_CACHE_PREFIX = "murl_";
const urlCR = (k) => {
  try {
    const r = sessionStorage.getItem(URL_CACHE_PREFIX + k);
    if (!r) return null;
    const { url, exp } = JSON.parse(r);
    if (Date.now() > exp) {
      sessionStorage.removeItem(URL_CACHE_PREFIX + k);
      return null;
    }
    return url;
  } catch {
...
const urlCW = (k, url) => {
  try {
    sessionStorage.setItem(URL_CACHE_PREFIX + k, JSON.stringify({ url, exp: Date.now() + URL_CACHE_TTL }));
  } catch {
  }
};
const EXPIRABLE = [
  { name: "pexels", test: (u) => u.includes("videos.pexels.com/video-files/"), extractId: (u) => u.match(/video-files\/(\d+)\//)?.[1] || null, resolve: async () => null },
  { name: "pixabay", test: (u) => /cdn\.pixabay\.com\/video\/\d{4}\/\d{2}\/\d{2}\//.test(u), extractId: (u) => u.match(/\/(\d+)-\d+_/)?.[1] || null, resolve: async (id) => {
    const r = await axiosClient.get(`/api/proxy/video?id=${id}&source=pixabay`);
    return r.data?.url || r.data?.videoUrl || null;
  } }
];
...
const getMediaUrls = (p) => {
  const all = [...Array.isArray(p.media) ? p.media : p.media ? [p.media] : [], ...Array.isArray(p.images) ? p.images : p.images ? [p.images] : [], p.videoUrl, p.embedUrl, p.thumbnail];
  return all.filter(Boolean).map((m) => typeof m === "string" ? m : m?.url).filter(Boolean);
};
const getResolvable = (p) => {
  if (p?.externalId) {
    for (const s of EXPIRABLE) {
      const id = s.extractId(p.externalId) || s.name === "pexels" && p.externalId.match(/^pexels_(\d+)$/)?.[1] || null;
      if (id) return { source: s, id };
    }
  }
  for (const url of [p?.videoUrl, p?.embedUrl].filter(Boolean)) {
    const s = expSrc(url);
    if (s) {
...
const resolvePost = async (post) => {
  try {
    ...
    if (!expUrls.length) {
      const r = getResolvable(post);
      if (!r) return post;
      const c = urlCR(`${r.source.name}_${r.id}`);
      if (c) return { ...post, videoUrl: c, _resolved: true };
      const f = await r.source.resolve(r.id);
      if (!f) return null;
      urlCW(`${r.source.name}_${r.id}`, f);
      return { ...post, videoUrl: f, _resolved: true };
    }
    const res = await Promise.all(expUrls.map(async (url) => {
      const s = expSrc(url), id = s.extractId(url);
      if (!id) return null;
      const k = `${s.name}_${id}`, c = urlCR(k);
      if (c) return c;
      try {
        const f = await s.resolve(id);
        if (f) urlCW(k, f);
        return f;
      } catch {
        return null;
    ...
    expUrls.forEach((orig, i) => {
      ...
      if (Array.isArray(post.media)) out.media = post.media.map((m) => {
        const u = typeof m === "string" ? m : m?.url;
        return u === orig ? typeof m === "string" ? res[i] : { ...m, url: res[i] } : m;
      });
      if (Array.isArray(post.images)) out.images = post.images.map((m) => {
        const u = typeof m === "string" ? m : m?.url;
        return u === orig ? typeof m === "string" ? res[i] : { ...m, url: res[i] } : m;
      });
    });
    ...
    while (i < posts.length) {
      ...
      if (onPartialResult && resolved % 5 === 0) {
        ...
        await new Promise((r) => {
          ...
          typeof requestIdleCallback !== "undefined" ? requestIdleCallback(r, { timeout: 100 }) : setTimeout(r, 0);
        });
      }
    }
  };
...
const isValidPost = (p) => {
  ...
  const exp = media.filter((u2) => !!expSrc(u2));
  if (exp.length > 0) {
    const r = getResolvable(p);
    if (!r && exp.length === media.length) return false;
    return true;
  }
...
const FeedSkeleton = memo(({ isDarkMode }) => {
  ...
  return /* @__PURE__ */ jsxs(Fragment, { children: [
    /* @__PURE__ */ jsx("style", { children: `@keyframes igs{0%{background-position:-400px 0}100%{background-position:400px 0}}.igs{background:linear-gradient(90deg,${bg1} 25%,${bg2} 50%,${bg1} 75%);background-size:800px 100%;animation:igs 1.5s ease-in-out infinite}` }),
    [0, 1, 2].map((i) => /* @__PURE__ */ jsxs("div", { className: `${isDarkMode ? "bg-black" : "bg-white"} border-b ${isDarkMode ? "border-gray-800" : "border-gray-100"}`, children: [
      /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-2.5 px-3 py-2.5", children: [
        /* @__PURE__ */ jsx("div", { className: "igs w-9 h-9 rounded-full flex-shrink-0" }),
        /* @__PURE__ */ jsxs("div", { className: "flex-1 space-y-1.5", children: [
          /* @__PURE__ */ jsx("div", { className: "igs h-3 rounded-full w-32" }),
          /* @__PURE__ */ jsx("div", { className: "igs h-2.5 rounded-full w-20" })
        ] })
      ] }),
      /* @__PURE__ */ jsx("div", { className: "igs w-full", style: { aspectRatio: "1/1" } }),
      /* @__PURE__ */ jsx("div", { className: "flex gap-2 px-3 py-2.5", children: [0, 1, 2].map((j) => /* @__PURE__ */ jsx("div", { className: "igs w-7 h-7 rounded" }, j)) }),
      /* @__PURE__ */ jsxs("div", { className: "px-3 pb-3 space-y-2", children: [
        /* @__PURE__ */ jsx("div", { className: "igs h-3 rounded-full w-28" }),
        /* @__PURE__ */ jsx("div", { className: "igs h-3 rounded-full w-full" }),
        /* @__PURE__ */ jsx("div", { className: "igs h-3 rounded-full w-2/3" })
      ] })
    ] }, i))
  ] });
});
FeedSkeleton.displayName = "FeedSkeleton";
const NewBanner = memo(({ count, onClick, topOffset }) => /* @__PURE__ */ jsx(AnimatePresence, { children: count > 0 && /* @__PURE__ */ jsx(
  motion.div,
  {
    ...
    children: /* @__PURE__ */ jsxs(
      "button",
      {
        ...
        children: [
          /* @__PURE__ */ jsx(ArrowUpIcon, { className: "w-3.5 h-3.5 opacity-80" }),
          /* @__PURE__ */ jsx("span", { children: count === 1 ? "1 nouveau post disponible" : `${count} nouveaux posts disponibles` })
        ]
      }
    )
  }
...
const AllSeenDivider = memo(({ isDarkMode }) => /* @__PURE__ */ jsxs("div", { className: `flex items-center gap-3 px-4 py-5 ${isDarkMode ? "bg-black" : "bg-white"}`, children: [
  /* @__PURE__ */ jsx("div", { className: `flex-1 h-px ${isDarkMode ? "bg-gray-800" : "bg-gray-200"}` }),
  /* @__PURE__ */ jsx("span", { className: `text-[11px] font-medium px-3 py-1 rounded-full ${isDarkMode ? "bg-gray-900 text-gray-500 border border-gray-800" : "bg-gray-50 text-gray-400 border border-gray-200"}`, children: "Vous avez tout vu \xB7 On recommence" }),
  /* @__PURE__ */ jsx("div", { className: `flex-1 h-px ${isDarkMode ? "bg-gray-800" : "bg-gray-200"}` })
] }));
AllSeenDivider.displayName = "AllSeenDivider";
const FetchingMore = memo(({ isDarkMode }) => /* @__PURE__ */ jsxs("div", { className: `flex items-center justify-center py-6 gap-2 ${isDarkMode ? "bg-black" : "bg-white"}`, children: [
  /* @__PURE__ */ jsx("div", { className: `w-4 h-4 border-2 rounded-full animate-spin ${isDarkMode ? "border-gray-700 border-t-gray-400" : "border-gray-200 border-t-gray-400"}` }),
  /* @__PURE__ */ jsx("span", { className: `text-xs ${isDarkMode ? "text-gray-600" : "text-gray-400"}`, children: "Chargement de nouveaux posts\u2026" })
] }));
...
const Toast = memo(({ message, type = "info", onClose }) => {
  ...
  return /* @__PURE__ */ jsx("div", { className: `fixed bottom-6 left-1/2 -translate-x-1/2 z-50 px-5 py-2.5 rounded-full shadow-xl text-[13px] font-medium text-white whitespace-nowrap ${type === "error" ? "bg-red-500" : type === "success" ? "bg-green-500" : "bg-gray-900"}`, children: message });
});
Toast.displayName = "Toast";
const ALLOWED_CATEGORIES = /* @__PURE__ */ new Set(["genieCivil", "technologie", "environnement"]);
const CATEGORY_META = {
...
const InfoPostCard = memo(({ article, isDarkMode, onClick, showToast }) => {
  ...
  const shareArticle = async (e) => {
    ...
    if (!shared) {
      ...
      showToast?.(
        copied ? "Lien de l'information copi\xE9" : "Impossible de partager cette information",
        copied ? "success" : "error"
      );
    } else {
      showToast?.("Information pr\xEAte \xE0 \xEAtre partag\xE9e", "success");
    }
  ...
  return /* @__PURE__ */ jsxs(
    ...
    {
      onClick,
      className: `relative w-full max-w-[630px] mx-auto cursor-pointer border-b transition-colors ${isDarkMode ? "bg-black border-gray-900" : "bg-white border-gray-100"}`,
      children: [
        /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-3 px-3 py-3", children: [
          /* @__PURE__ */ jsx(
            "div",
            {
              ...
              children: /* @__PURE__ */ jsx(NewspaperIcon, { className: "h-5 w-5" })
            }
          ),
          /* @__PURE__ */ jsxs("div", { className: "min-w-0 flex-1", children: [
            /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-1.5", children: [
              /* @__PURE__ */ jsx("span", { className: `truncate text-sm font-semibold ${isDarkMode ? "text-white" : "text-gray-900"}`, children: article.source || "Information" }),
              /* @__PURE__ */ jsx("span", { className: `rounded-full px-2 py-0.5 text-[10px] font-bold text-white bg-gradient-to-r ${meta.gradient}`, children: meta.label })
            ] }),
            /* @__PURE__ */ jsx("span", { className: "text-xs text-gray-500", children: fmtDate(article.publishedAt) })
          ] }),
          /* @__PURE__ */ jsx(
            "button",
            {
              ...
              children: /* @__PURE__ */ jsx(EyeSlashIcon, { className: "h-5 w-5" })
            }
          )
        ] }),
        /* @__PURE__ */ jsxs("div", { className: "px-3 pb-3", children: [
          /* @__PURE__ */ jsx("h3", { className: `text-[15px] font-bold leading-snug ${isDarkMode ? "text-white" : "text-gray-900"}`, children: article.title }),
          article.description && /* @__PURE__ */ jsx("p", { className: `mt-1.5 line-clamp-3 text-[13px] leading-relaxed ${isDarkMode ? "text-gray-300" : "text-gray-700"}`, children: article.description })
        ] }),
        article.image && !imgErr ? /* @__PURE__ */ jsx("div", { className: "relative w-full overflow-hidden bg-gray-200 dark:bg-gray-900", style: { aspectRatio: "1/1" }, children: /* @__PURE__ */ jsx("img", { src: article.image, alt: article.title, className: "w-full h-full object-cover", loading: "lazy", onError: () => setImgErr(true) }) }) : /* @__PURE__ */ jsx("div", { className: `w-full flex items-center justify-center bg-gradient-to-br ${meta.gradient}`, style: { aspectRatio: "1/1" }, children: /* @__PURE__ */ jsx(NewspaperIcon, { className: "h-20 w-20 text-white/40" }) }),
        /* @__PURE__ */ jsxs("div", { className: "flex items-center justify-between px-3 py-2", children: [
          /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-4", children: [
            /* @__PURE__ */ jsxs(
              "button",
              {
                ...
                children: [
                  /* @__PURE__ */ jsx(ArrowTopRightOnSquareIcon, { className: "h-5 w-5" }),
                  /* @__PURE__ */ jsx("span", { children: "Lire" })
                ]
              }
            ),
            /* @__PURE__ */ jsxs(
              "button",
              {
                ...
                className: `flex items-center gap-2 text-sm font-semibold transition-colors ${isDarkMode ? "text-gray-300 hover:text-orange-400" : "text-gray-700 hover:text-orange-500"}`,
                children: [
                  /* @__PURE__ */ jsx(ShareIcon, { className: "h-5 w-5" }),
          ...
          /* @__PURE__ */ jsx(
            "button",
            {
              ...
              className: `flex items-center gap-2 text-sm font-semibold transition-colors ${isDarkMode ? "text-gray-300 hover:text-orange-400" : "text-gray-700 hover:text-orange-500"}`,
              children: saved ? /* @__PURE__ */ jsx(BookmarkSolid, { className: "h-5 w-5 text-orange-500" }) : /* @__PURE__ */ jsx(BookmarkIcon, { className: "h-5 w-5" })
            }
          )
        ] })
      ]
...
const PostCardWrapper = memo(({ post, index, onVisible, onDwell, onSkip, ...rest }) => {
  ...
  const displayPost = useMemo(() => post?._displayPosition === index ? post : { ...post, _displayPosition: index }, [post, index]);
  useEffect(() => {
  ...
  return /* @__PURE__ */ jsx("div", { ref: wrapRef, children: /* @__PURE__ */ jsx(PostCard, { post: displayPost, priority: index === 0, ...rest }) });
});
...
const Feed = ({
  ...
  useEffect(() => {
    onNeedMorePostsRef.current = onNeedMorePosts;
  }, [onNeedMorePosts]);
  useEffect(() => {
    postsRef.current = posts;
  }, [posts]);
  useEffect(() => {
    onPostsSeenRef.current = onPostsSeen;
  }, [onPostsSeen]);
  useEffect(() => {
  ...
  const taggedCache = useRef(/* @__PURE__ */ new Map());
  ...
  const loadMore = useCallback(() => {
    ...
    const ctx2 = getScoringContext();
    tagged.forEach((p) => {
      ctx2.fatigue.record(p);
      ctx2.trend.record(p);
      ctx2.session.recordView();
    });
  ...
  useEffect(() => {
    ...
    if (scrollEl) {
      ...
      setTimeout(() => clearInterval(poll), 2e3);
    }
    return () => cleanup();
  }, [scrollContainerRef]);
  useEffect(() => {
  ...
  const handleDwell = useCallback((post, ms) => {
    ...
    const label = ms > 8e3 ? 1 : ms > 3e3 ? 0.5 : 0.2;
  ...
  const renderItem = useCallback((post, index) => {
    ...
    const newsItem = newsSlot >= 0 && articles.length > 0 ? articles[newsSlot % articles.length] : null;
    return /* @__PURE__ */ jsxs(Fragment, { children: [
      /* @__PURE__ */ jsx(
        PostCardWrapper,
        {
      ...
      index > 0 && index % SUGGEST_PROFILE_EVERY === 0 && /* @__PURE__ */ jsx(
      ...
      index > 0 && index % SUGGEST_ACCOUNTS_EVERY === 0 && index % SUGGEST_PROFILE_EVERY !== 0 && /* @__PURE__ */ jsx(
        SuggestedAccounts,
        {
          isDarkMode,
  ...
  if (isLoading && !displayedPosts.length) return /* @__PURE__ */ jsx(FeedSkeleton, { isDarkMode });
  if (!isLoading && !displayedPosts.length) return /* @__PURE__ */ jsxs("div", { className: "flex flex-col items-center justify-center py-24 gap-3", children: [
    /* @__PURE__ */ jsx("div", { className: `w-16 h-16 rounded-full flex items-center justify-center ${isDarkMode ? "bg-gray-900" : "bg-gray-100"}`, children: /* @__PURE__ */ jsxs("svg", { className: `w-8 h-8 ${isDarkMode ? "text-gray-600" : "text-gray-300"}`, fill: "none", viewBox: "0 0 24 24", stroke: "currentColor", strokeWidth: 1.2, children: [
      /* @__PURE__ */ jsx("rect", { x: "3", y: "3", width: "18", height: "18", rx: "3" }),
      /* @__PURE__ */ jsx("circle", { cx: "8.5", cy: "8.5", r: "1.5" }),
      /* @__PURE__ */ jsx("path", { d: "M21 15l-5-5L5 21" })
    ] }) }),
    /* @__PURE__ */ jsx("p", { className: `text-sm ${isDarkMode ? "text-gray-600" : "text-gray-400"}`, children: "Aucune publication" })
  ] });
  return /* @__PURE__ */ jsxs(Fragment, { children: [
    /* @__PURE__ */ jsx(NewBanner, { count: newPostsCount, onClick: onShowNewPosts, topOffset }),
    /* @__PURE__ */ jsx(
      VirtualFeed,
      {
    ...
    showAllSeen && /* @__PURE__ */ jsx(AllSeenDivider, { isDarkMode }),
    isFetchingAPI && /* @__PURE__ */ jsx(FetchingMore, { isDarkMode }),
    /* @__PURE__ */ jsx("div", { ref: sentinelRef, className: "h-10 flex items-center justify-center", "aria-hidden": "true", children: displayedPosts.length > 0 && !isFetchingAPI && /* @__PURE__ */ jsx("div", { className: `w-1 h-1 rounded-full ${isDarkMode ? "bg-gray-800" : "bg-gray-200"}` }) }),
    hasMoreFromAPI && /* @__PURE__ */ jsx(
      "div",
      {
        ref: (el) => {
    ...
    selectedArticle && /* @__PURE__ */ jsx(Suspense, { fallback: null, children: /* @__PURE__ */ jsx(
      ArticleReaderModal,
      {
...
const Home = ({ openStoryViewer: openStoryViewerProp, searchQuery = "" }) => {
  ...
  const followingIdsRef = useRef(/* @__PURE__ */ new Set());
  ...
  useEffect(() => {
    fetchNextPageRef2.current = fetchNextPage;
  }, [fetchNextPage]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!hasCachedPosts) {
      persistedSeenIdsRef.current = /* @__PURE__ */ new Set();
      saveSeenIds(/* @__PURE__ */ new Set());
    }
  ...
  }, [newsGCLen, newsTechLen, newsEnvLen]);
  ...
  const addLivePosts = useCallback((list, options = {}) => {
    const incoming = Array.from(
      new Map(
        (Array.isArray(list) ? list : []).filter((p) => p?._id && !isPostHidden(p)).map(({ _displayKey, ...rest }) => [rest._id, rest])
      ).values()
    );
    if (!incoming.length) return false;
    const incomingIds = new Set(incoming.map((p) => p._id));
    const kept = options.resetPool ? [] : livePostsRef.current.filter((p) => p?._id && !incomingIds.has(p._id));
    livePostsRef.current = options.prepend ? [...incoming, ...kept].slice(0, MAX_POOL) : [...kept, ...incoming].slice(0, MAX_POOL);
  ...
  const guardedRefetchFirstPage = useCallback(async (options = {}) => {
    const { force = false, minInterval = 2e4, refetchOptions = {} } = options;
    const now = Date.now();
  ...
  const handleNeedMorePosts = useCallback(async () => {
    ...
    try {
      ...
      if (now - silentLastFetchRef.current >= SILENT_COOLDOWN_MS / 2) {
        ...
        const result = await guardedRefetchFirstPage();
    ...
    try {
      ...
      if (!window.__fallbackPostsRoutePromise__) {
        ...
        window.__fallbackPostsRoutePromise__ = probeUid ? (async () => {
          try {
            await axiosClient.get(`/users/${probeUid}/posts?limit=1&page=1`, { signal: ctrl.signal });
            return "user_posts";
          } catch (e1) {
            if (e1.response?.status !== 404) return "user_posts";
            try {
              await axiosClient.get(`/posts?userId=${probeUid}&limit=1`, { signal: ctrl.signal });
              return "posts_filter";
            } catch (e2) {
              if (e2.response?.status !== 404) return "posts_filter";
              try {
                await axiosClient.get(`/posts/user/${probeUid}?limit=1`, { signal: ctrl.signal });
                return "posts_user";
              } catch {
                return "none";
              }
  ...
  useEffect(() => {
    fetchFallbackRef.current = fetchFallbackPosts;
  }, [fetchFallbackPosts]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const h = () => {
      ...
      if (rc < FALLBACK_THRESHOLD) {
        ...
        const filtered2 = c.filter((p) => !p._shouldIsolateFromHomeFeed);
        if (filtered2.length > 0) addLivePosts(filtered2);
      }
    };
  ...
  useEffect(() => {
    ...
    const filtered2 = livePostsRef.current.filter((p) => !ids.has(p._id));
    const cleanRaw = Array.from(
      new Map(visibleRawPosts.filter((p) => p?._id).map((p) => [p._id, p])).values()
    ).map(({ _displayKey, ...rest }) => rest);
    livePostsRef.current = [...cleanRaw, ...filtered2].slice(0, MAX_POOL);
    saveLivePool(livePostsRef.current);
    startTransition(() => setLivePostsVer((v) => v + 1));
  }, [rawPosts]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (resolved.length > 0 && !latestId.current) latestId.current = resolved[0]._id;
  }, [resolved]);
  ...
  const handleRefresh = useCallback(async () => {
    ...
    fallbackFetchedRef.current = false;
    try {
      if (postsLoading) await new Promise((resolve) => {
        const maxWait = setTimeout(resolve, 2e3);
  ...
  const handleScrollProgress = useCallback(async (ratio) => {
    ...
    if (ratio >= PREFETCH_THRESHOLD && !prefetchTriggeredRef.current) {
      ...
      silentLastFetchRef.current = now;
      try {
        const r = await guardedRefetchFirstPage();
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (Date.now() - lastHiddenAt < 5e3) return;
      try {
        const r = await guardedRefetchFirstPage({ minInterval: POLL_INTERVAL - 1e3 });
        const fp = r?.posts || [];
  ...
  useEffect(() => {
    ...
    const onStart = (e) => {
      ...
      if (st <= 2 && canPull.current) touchStartY.current = e.touches[0].clientY;
    };
    const onMove = (e) => {
      ...
      if (pd > 10) {
        ...
        if (now - lu >= 40) {
          ...
          raf = requestAnimationFrame(() => setPullDist(pullDistRef.current));
        }
        if (pd > 50 && e.cancelable) try {
          e.preventDefault();
        } catch {
        }
      }
    ...
    const onEnd = () => {
      if (raf) cancelAnimationFrame(raf);
      pullDistRef.current > PTR && !isPulling.current ? trigger() : reset();
  ...
  useEffect(() => {
    ...
    const h = (e) => {
      ...
      switch (action) {
        case "like":
          _ctx.tracker.onLike(post, position);
          _ctx.userEmb.update(postVec, 1);
          _ctx.ltm.update(post, 0.6);
          _ctx.calib.recordInteraction(isControl);
          break;
        case "comment":
          _ctx.tracker.onComment(post, position);
          _ctx.userEmb.update(postVec, 1.2);
          _ctx.ltm.update(post, 0.8);
          _ctx.calib.recordInteraction(isControl);
          break;
        case "share":
          _ctx.tracker.onShare(post, position);
          _ctx.userEmb.update(postVec, 1.5);
          _ctx.ltm.update(post, 1);
          _ctx.calib.recordInteraction(isControl);
          break;
        case "save":
          _ctx.tracker.onSave(post, position);
          _ctx.userEmb.update(postVec, 0.8);
          _ctx.ltm.update(post, 0.5);
          _ctx.calib.recordInteraction(isControl);
          break;
        case "hide":
        case "not_interested":
          _ctx.tracker.onHide(post, position);
          _ctx.userEmb.update(postVec, -1.5);
          _ctx.ltm.update(post, -0.8);
          break;
        case "report":
          _ctx.tracker.onHide(post, position);
          _ctx.userEmb.update(postVec, -2);
          _ctx.ltm.update(post, -1);
          break;
        default:
          break;
  ...
  useEffect(() => {
    return () => {
      console.log("\u{1F9F9} [Home] Component unmount - nettoyage du feed");
      livePostsRef.current = [];
      persistedSeenIdsRef.current = /* @__PURE__ */ new Set();
      latestId.current = null;
      currentApiPageRef.current = 1;
  ...
  return /* @__PURE__ */ jsxs("div", { className: `flex flex-col ${bg}`, style: { height: "100%", overflow: "hidden" }, children: [
    /* @__PURE__ */ jsxs(
      "div",
      {
        ref: scrollRef,
        ...
        ` }),
          /* @__PURE__ */ jsx(AnimatePresence, { children: (pullDist > 8 || isRefreshing) && /* @__PURE__ */ jsx(
            motion.div,
            {
              ...
              children: /* @__PURE__ */ jsx(
                ArrowPathIcon,
                {
                  ...
                  style: { transform: isRefreshing ? void 0 : `rotate(${Math.min(pullDist / PTR, 1) * 270}deg)` }
                }
              )
            }
          ) }),
          /* @__PURE__ */ jsxs("div", { className: "flex gap-0 xl:gap-6 max-w-[1100px] mx-auto", children: [
            /* @__PURE__ */ jsxs("div", { className: "flex-1 min-w-0 xl:max-w-[630px]", children: [
              /* @__PURE__ */ jsx("div", { className: `${bg} border-b ${border}`, style: { height: 92, overflow: "hidden" }, children: /* @__PURE__ */ jsx(
                StoryContainer,
                {
            ...
            /* @__PURE__ */ jsxs(
              "div",
              {
                ...
                children: [
                  /* @__PURE__ */ jsxs("div", { className: `rounded-2xl overflow-hidden border ${isDarkMode ? "border-gray-800 bg-gray-900" : "border-gray-200 bg-gray-50"}`, children: [
                    /* @__PURE__ */ jsx("div", { className: `px-4 pt-3 pb-2 border-b ${isDarkMode ? "border-gray-800" : "border-gray-100"}`, children: /* @__PURE__ */ jsx("p", { className: `text-[13px] font-bold ${isDarkMode ? "text-gray-200" : "text-gray-800"}`, children: "Suggestions pour vous" }) }),
                    /* @__PURE__ */ jsx(
                      SuggestedAccounts,
                      {
                        isDarkMode,
                  ...
                  /* @__PURE__ */ jsxs("div", { className: `rounded-2xl overflow-hidden border ${isDarkMode ? "border-gray-800 bg-gray-900" : "border-gray-200 bg-gray-50"}`, children: [
                    /* @__PURE__ */ jsx("div", { className: `px-4 pt-3 pb-2 border-b ${isDarkMode ? "border-gray-800" : "border-gray-100"}`, children: /* @__PURE__ */ jsx("p", { className: `text-[13px] font-bold ${isDarkMode ? "text-gray-200" : "text-gray-800"}`, children: "\xC0 d\xE9couvrir" }) }),
                    /* @__PURE__ */ jsx(SuggestedPostPreview, { isDarkMode, userPool: suggestedUsers, slotIndex: 0 })
                  ] })
                ]
              }
            )
    ...
    /* @__PURE__ */ jsx(GlobalModalManager, {}),
    /* @__PURE__ */ jsx(AnimatePresence, { children: showScrollTop && /* @__PURE__ */ jsx(
      motion.button,
      {
        ...
        children: /* @__PURE__ */ jsx(ArrowUpIcon, { className: "w-5 h-5" })
      }
    ) }),
    ...
    /* @__PURE__ */ jsx(AnimatePresence, { children: showCreator && /* @__PURE__ */ jsx(Suspense, { fallback: null, children: /* @__PURE__ */ jsx(StoryCreator, { onClose: () => setShowCreator(false) }) }) }),
    ...
    /* @__PURE__ */ jsx(AnimatePresence, { children: toast && /* @__PURE__ */ jsx(Toast, { message: toast.message, type: toast.type, onClose: () => setToast(null) }) })
  ] });
};
var Home_default = memo(Home);
export {
