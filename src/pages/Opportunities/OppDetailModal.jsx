 ...
 *  - En-tête avec TypeIcon, titre, badge type, badge nouveau
 *  - Méta : entreprise, localisation, date, source
 *  - Description (si disponible) ou message de redirection
 *  - Tags sous forme de pills
 *  - Date de clôture (AO)
 *  - Bouton "Postuler / Voir l'offre" → ouvre sourceUrl dans un nouvel onglet
 *
 * Rendu via ReactDOM.createPortal → pas de problème de z-index
 */
...
// ─── Config types (dupliqué ici pour que la modal soit autonome) ──────────────
const TYPE_CONFIG = {
  emploi: {
    label:   "Emploi",
    color:   "#3b82f6",
...
const OppDetailModal = memo(({ opp, isDarkMode, onClose }) => {
  ...
  const modal = (
    <AnimatePresence>
      ...
      >
        {/* Sheet panel — monte depuis le bas */}
        <motion.div
          key="modal-panel"
          initial={{ y: "100%" }}
          animate={{ y: 0 }}
