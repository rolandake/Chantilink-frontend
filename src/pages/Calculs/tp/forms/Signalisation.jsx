...
import { 
  TrafficCone, ShieldAlert, Navigation, MoveHorizontal, Layers, Construction, 
  Save, Trash2, History, Ruler, Banknote, Info,
...
const EQUIPEMENTS_CONFIG = {
  ...
  feux:       { label: "Feux Tricolores",  icon: <ShieldAlert className="w-5 h-5"/>,   unit: "u",  color: "#ef4444" },
...
const safeLoadHistorique = () => {
  try {
    ...
    return parsed.filter(item => item && item.id && typeof item.totalGlobal === "number");
  } catch {
...
export default function EquipementsDeSignalisation({ currency = "XOF", onCostChange, onMateriauxChange }) {
  ...
  const results = useMemo(() => {
    ...
    if (selectedType === "marquage") {
      poidsTotalT = (qte * (config.dosageKgM2 ?? 0) * margin) / 1000;
    } else if (selectedType === "glissieres" || selectedType === "barriers") {
      poidsTotalT = (qte * (config.poidsML ?? 0) * margin) / 1000;
    }
  ...
  const handleSave = () => {
    ...
    // ✅ Champs à la racine — cohérence avec la lecture dans le template
    const newEntry = {
      id:             Date.now(),
      ...
      coutPose:       inputs.coutPose,
      margePerte:     inputs.margePerte,
      totalFourniture: results.totalFourniture,
      totalPose:       results.totalPose,
      totalGlobal:     results.totalGlobal,
      poidsTotalT:     results.poidsTotalT,
    };
          ...
          <div className="lg:col-span-5 flex flex-col gap-5">
            ...
            <div className="bg-gray-800/50 border border-gray-700 rounded-3xl p-6 shadow-xl space-y-6">
              <h3 className="flex items-center gap-2 text-xs font-bold text-orange-400 uppercase tracking-widest">
                {/* ✅ FIX — Crosshair au lieu de Target */}
                <Crosshair className="w-4 h-4" /> Paramètres {EQUIPEMENTS_CONFIG[selectedType].label}
              </h3>
            ...
            {historique.length > 0 && (
              <div className="bg-gray-800/30 rounded-2xl border border-gray-700/50 overflow-hidden">
                <div className="px-4 py-2 bg-gray-800/50 border-b border-gray-700/50 flex justify-between items-center text-xs">
                  <h4 className="font-bold text-gray-500 uppercase tracking-widest flex items-center gap-2">
                    <History className="w-3 h-3" /> Historique
                  </h4>
                  <button onClick={handleClear} className="text-red-400 hover:underline uppercase text-[10px]">Vider</button>
                </div>
                    ...
                    <div key={item.id} className="flex justify-between items-center p-3 border-b border-gray-700/30 hover:bg-gray-700/40 transition-colors">
                      <div className="text-xs">
                        ...
                        <span className="font-medium uppercase">
                          {item.label} — {item.quantite} {EQUIPEMENTS_CONFIG[item.type]?.unit ?? ""}
                        </span>
                      </div>
                      {/* ✅ FIX — item.totalGlobal à la racine */}
                      <span className="text-sm font-bold text-orange-400">
                        {(item.totalGlobal ?? 0).toLocaleString()} {currency}
                      </span>
                    </div>
                  ))}
                </div>
...
const ResultCard = ({ label, value, unit, color, bg, border, icon }) => (
  <div className={`rounded-2xl p-4 flex flex-col justify-center items-center text-center ${bg} ${border ? "border border-gray-700" : ""}`}>
    ...
    <span className={`text-xl font-black ${color}`}>
      {Number(value).toLocaleString()} <span className="text-xs font-normal text-gray-500 lowercase">{unit}</span>
    </span>
  </div>
);
