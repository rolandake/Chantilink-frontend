...
import { 
  ...
  Save, Trash2, History, Ruler, Banknote, Brush
} from "lucide-react";

ChartJS.register(ArcElement, Tooltip, Legend);

const STORAGE_KEY = "finitions-batiment-history";

// Configuration des types de finitions
...
export default function Finitions({ currency = "XOF", onTotalChange, onMateriauxChange }) {
  ...
  const results = useMemo(() => {
    ...
    // Calcul : Surface * (PrixMat * Couches + PrixMO) 
    // Note : La main d'œuvre est souvent au m² fini, donc pas multipliée par les couches, 
    // mais le matériel (peinture) l'est.
  ...
  const currentTheme = FINITION_TYPES.find(t => t.id === selectedType) || FINITION_TYPES[0];
  ...
  return (
    <div className="w-full h-full flex flex-col bg-gray-900 text-gray-100 overflow-hidden relative">
      ...
      {/* Header */}
      <div className="flex-shrink-0 px-6 py-4 border-b border-gray-800 flex justify-between items-center bg-gray-900/50 backdrop-blur-sm gap-4">
            ...
            <div className="flex-1 bg-gray-800 rounded-2xl p-6 border border-gray-700 shadow-xl flex flex-col md:flex-row gap-8 items-center relative overflow-hidden">
               ...
               <div className="w-40 h-40 flex-shrink-0 relative">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                     <span className="text-sm font-bold text-violet-400">Total</span>
                  </div>
               </div>
                ...
                <div className="overflow-y-auto max-h-[180px] p-2 space-y-2">
                  {historique.map((item, index) => {
                    ...
                    return (
                      <div 
                        key={item.id} 
                        className="flex justify-between items-center bg-gray-700/30 p-2 rounded hover:bg-gray-700/50 transition border border-transparent hover:border-violet-500/30"
                      >
                        <div className="flex items-center gap-3">
