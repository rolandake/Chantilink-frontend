...
const STORAGE_KEY = "chutes-tension-history";
const RESISTIVITE = 0.0175; // cuivre

export default function ChutesTension({ onNext = () => {} }) {
  ...
  const handleSave = () => {
    if (!L || !I || !U || !S) {
      alert("⚠️ Saisir tous les champs correctement.");
      return;
    }
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-gray-800 rounded-lg shadow-lg text-gray-100">
      <h2 className="text-2xl font-bold text-orange-400 mb-6 text-center">⚡ Chutes de Tension</h2>
      ...
      <div className="grid grid-cols-2 gap-5 mb-6">
        ...
        <div>
          <label className="block mb-1 font-semibold text-orange-400">Section (mm²)</label>
          <input
            type="number"
            min="0"
            value={section}
      ...
      <div className="bg-gray-700 p-4 rounded shadow-inner mb-6">
        <p className="text-lg">
          🔽 Chute de tension :{" "}
          <span className={chuteTension > 5 ? "text-red-400 font-bold" : "text-green-400 font-bold"}>
            {chuteTension.toFixed(2)}%
          </span>{" "}
          (max. 5%)
        </p>
      </div>
          ...
          {historique.map((item) => (
            ...
            >
              <div>
                ...
                <p className="font-bold text-orange-400">Chute : {item.chute}%</p>
              </div>
              <button
