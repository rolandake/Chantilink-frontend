...
const STORAGE_KEY = "mise-en-service-history";

export default function MiseEnService({ onCostChange = () => {} }) {
  const [nombreJour, setNombreJour] = useState("");
  const [coutParJour, setCoutParJour] = useState("");
  const [coutMainOeuvre, setCoutMainOeuvre] = useState("");
  const [autresCouts, setAutresCouts] = useState("");
  const [historique, setHistorique] = useState([]);

  const totalJours = (parseFloat(nombreJour) || 0) * (parseFloat(coutParJour) || 0);
  const total =
    totalJours +
  ...
  const handleSave = () => {
    ...
    const entry = {
      ...
      coutMainOeuvre,
      autresCouts,
      totalJours: totalJours.toFixed(2),
      total: total.toFixed(2),
    };
    setHistorique([entry, ...historique]);
    alert("✅ Coûts mise en service enregistrés !");
  };
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-gray-900 rounded-lg shadow-lg text-gray-100 font-sans">
      <h3 className="text-xl font-bold text-orange-400 mb-4">Mise en Service</h3>

      <div className="grid grid-cols-3 gap-4 mb-4">
        {[
          { label: "Nombre de jours", value: nombreJour, setter: setNombreJour },
          { label: "Coût par jour (XOF)", value: coutParJour, setter: setCoutParJour },
          ...
          {historique.map((item) => (
            ...
            >
              <div>
                ...
                <p>Nombre de jours : {item.nombreJour}</p>
                <p>Coût par jour : {item.coutParJour} XOF</p>
