...
// ✅ FIX — lecture sécurisée du localStorage.
// JSON.parse peut lancer une exception si les données sont corrompues
// (ex: ancienne version de l'app, coupure réseau pendant l'écriture).
// Sans try/catch, toute la page CalculsPage devient blanche.
const safeLoadCosts = () => {
  try {
    const saved  = localStorage.getItem(STORAGE_KEY);
    if (!saved) return {};
    const parsed = JSON.parse(saved);
    // Vérifie que c'est bien un objet plat { stepId: number }
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) return {};
    // Assainit les valeurs : garde seulement les nombres >= 0
    const safe = {};
    Object.entries(parsed).forEach(([k, v]) => {
      const n = Number(v);
      if (Number.isFinite(n) && n >= 0) safe[k] = n;
    });
    return safe;
  } catch {
    // Données corrompues → repart de zéro et nettoie le storage
...
export default function FerroviaireForm() {
  ...
  const [costs,            setCosts]            = useState(safeLoadCosts);   // ✅ FIX appliqué ici
  const [quantitiesByStep, setQuantitiesByStep] = useState({});

  // ✅ Sauvegarde automatique — sécurisée également
  useEffect(() => {
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify(costs));
    } catch {
      // Storage plein ou mode privé → on ignore silencieusement
    }
  }, [costs]);
  ...
  const handleCostChange = useCallback((stepId, value) => {
    setCosts(prev => {
      ...
      if (prev[stepId] === n) return prev;
      return { ...prev, [stepId]: n };
    });
  }, []);
  ...
  const totalGeneral = useMemo(
    () => Object.values(costs).reduce((acc, v) => acc + (Number(v) || 0), 0),
    [costs]
  );

  const progress = useMemo(
    () => (Object.values(costs).filter(v => v > 0).length / stepsConfig.length) * 100,
    [costs]
  );
    ...
    <div className="flex h-[calc(100vh-64px)] w-full bg-gray-950 text-white overflow-hidden font-sans">
      ...
      <aside className="hidden lg:flex flex-col w-72 bg-gray-900 border-r border-gray-800 shadow-2xl z-20">
        <div className="p-6 border-b border-gray-800">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h1 className="text-xl font-black tracking-tighter uppercase italic">
              RailPro <span className="text-indigo-500">BE</span>
            </h1>
          </div>
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar">
          ...
          <div className="h-4" />
          <p className="px-3 text-[10px] font-black text-gray-600 uppercase tracking-[0.2em] mb-2">
            Phases du projet
          </p>
            ...
            <button
              ...
              className={`w-full flex items-center justify-between p-3 rounded-xl transition-all group ${
                selectedStep === step.id
                  ? "bg-gray-800 border border-indigo-500/50"
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className={`text-xs font-bold ${
                  selectedStep === step.id ? "text-white" : "text-gray-500"
                }`}>
                  {step.label}
                </span>
              </div>
          ...
          <button
            onClick={() => setSelectedStep("devisFerroviaire")}
            className="w-full flex items-center gap-3 p-3 rounded-xl text-gray-400 hover:bg-gray-800"
          >
        ...
        <div className="p-4 bg-gray-900 border-t border-gray-800">
          <div className="bg-indigo-600/10 border border-indigo-500/30 p-4 rounded-2xl">
            ...
            <div className="text-lg font-black text-white truncate">
              ...
              <span className="text-xs font-normal opacity-60">{currency}</span>
            </div>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col h-full bg-gray-950 relative overflow-hidden">
        ...
        <div className="lg:hidden flex items-center justify-between p-4 border-b border-gray-800 bg-gray-900 z-30">
          ...
          <select
            ...
            className="bg-gray-800 text-[10px] border-none rounded px-2 py-1 outline-none"
          >
              ...
              >
                <div className="text-center mb-12">
                  ...
                  <p className="text-gray-400">
                    Gérez l'ensemble de la chaîne de valeur : de l'étude d'impact à la maintenance.
                  </p>
                </div>
        ...
        <div className="flex-shrink-0 p-4 border-t border-gray-800 bg-gray-900/50 backdrop-blur-sm flex justify-center gap-4">
          ...
          <button
            onClick={handleReset}
            className="flex items-center gap-2 bg-gray-800 hover:bg-red-900/40 px-6 py-2.5 rounded-full font-bold text-sm transition-all border border-gray-700 text-red-400"
          >
