...
const STORAGE_KEY = "installation-signalisation-history";

export default function InstallationSignalisation({ onCostChange = () => {} }) {
  const [nombreSignaux, setNombreSignaux] = useState("");
  const [prixUnitaireSignal, setPrixUnitaireSignal] = useState("");
  const [coutInstallation, setCoutInstallation] = useState("");
  const [coutMainOeuvre, setCoutMainOeuvre] = useState("");
  const [historique, setHistorique] = useState([]);

  // Calcul du total
  const coutSignaux = (parseFloat(nombreSignaux) || 0) * (parseFloat(prixUnitaireSignal) || 0);
  const total =
    coutSignaux +
  ...
  const handleSave = () => {
    ...
    const entry = {
      ...
      coutInstallation,
      coutMainOeuvre,
      coutSignaux: coutSignaux.toFixed(2),
      total: total.toFixed(2),
    };
    setHistorique([entry, ...historique]);
    alert("✅ Coûts signalisation enregistrés !");
  };
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-gray-900 rounded-lg shadow-lg text-gray-100 font-sans">
      <h3 className="text-xl font-bold text-orange-400 mb-4">Installation Signalisation</h3>

      <div className="grid grid-cols-3 gap-4 mb-4">
        {[
          { label: "Nombre de signaux", value: nombreSignaux, setter: setNombreSignaux },
          { label: "Prix unitaire signal (XOF/unité)", value: prixUnitaireSignal, setter: setPrixUnitaireSignal },
          { label: "Coût installation (XOF)", value: coutInstallation, setter: setCoutInstallation },
          ...
          {historique.map((item) => (
            ...
            >
              <div>
                ...
                <p>Nombre de signaux : {item.nombreSignaux}</p>
                <p>Prix unitaire : {item.prixUnitaireSignal} XOF</p>
                <p>Coût installation : {item.coutInstallation} XOF</p>
