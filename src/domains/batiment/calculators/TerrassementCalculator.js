...
export class TerrassementCalculator extends BaseCalculator {
  validate() {
    return this.validatePositiveNumbers(['longueur', 'largeur', 'profondeur']);
  }

  calculate() {
    const { longueur, largeur, profondeur, prixUnitaire = 0, coutMainOeuvre = 0 } = this.inputs;
    
    // Calcul du volume
    const volume = parseFloat(longueur) * parseFloat(largeur) * parseFloat(profondeur) || 0;
