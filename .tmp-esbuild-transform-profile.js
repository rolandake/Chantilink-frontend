...
const isMockId = (id) => !isValidObjectId(id) && typeof id === "string" && (id.startsWith("user_") || id.startsWith("post_") || id.startsWith("mock_"));
const PROFILE_CACHE_TTL = 5 * 60 * 1e3;
const getProfilePostsCache = () => {
  if (!window.__profilePostsCache__) window.__profilePostsCache__ = /* @__PURE__ */ new Map();
...
const readAllCachedProfilePosts = (maxAge = PROFILE_CACHE_TTL) => {
  try {
    const cache = getProfilePostsCache();
    const now = Date.now();
...
const clearProfilePostsCache = () => {
  if (typeof window !== "undefined") {
    try {
      window.__profilePostsCache__ = /* @__PURE__ */ new Map();
      console.log("\u2705 [ProfilePage] Cache des posts du profil nettoy\xE9");
    } catch {
    }
  }
};
const normalizePost = (p) => {
  ...
  return {
    ...
    media: Array.isArray(p.media) ? p.media.map((m) => ({ url: m?.url || m?.path || m?.location || m, type: m?.type || p.mediaType || "image" })) : [],
    user: normalizedUser,
    ...
    textCardPalette: p.textCardPalette ?? void 0,
...
const LoadingSpinner = memo(({ darkMode = false, text = "Chargement..." }) => /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", padding: "48px 0", fontFamily: "'Sora','DM Sans',sans-serif" } }, /* @__PURE__ */ React.createElement("div", { style: {
...
} }), text && /* @__PURE__ */ React.createElement("p", { style: { marginTop: 14, color: darkMode ? "#6b7280" : "#9ca3af", fontSize: 14 } }, text), /* @__PURE__ */ React.createElement("style", null, `@keyframes spin { to { transform: rotate(360deg); } }`)));
LoadingSpinner.displayName = "LoadingSpinner";
const FollowButton = memo(({ isFollowing, isLoading, onClick, isDarkMode }) => /* @__PURE__ */ React.createElement(
  motion.button,
  {
    onClick,
    disabled: isLoading,
    ...
    style: {
      ...
      background: isFollowing ? isDarkMode ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.05)" : "linear-gradient(135deg,#f97316,#ec4899)",
  ...
  isLoading ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("span", { style: {
    ...
    borderRadius: "50%",
    animation: "spin 0.8s linear infinite"
  } }), "Chargement...") : isFollowing ? "Se d\xE9sabonner" : "S'abonner"
));
...
const EmptyPostsState = memo(({ isOwner, isDarkMode }) => /* @__PURE__ */ React.createElement(
  motion.div,
  {
  ...
  /* @__PURE__ */ React.createElement("div", { style: { fontSize: 52, marginBottom: 16 } }, "\u{1F4ED}"),
  /* @__PURE__ */ React.createElement("p", { style: { fontSize: 17, fontWeight: 700, color: isDarkMode ? "#d1d5db" : "#374151", marginBottom: 8 } }, isOwner ? "Tu n'as pas encore publi\xE9 de post" : "Aucun post pour l'instant"),
  /* @__PURE__ */ React.createElement("p", { style: { fontSize: 13, color: isDarkMode ? "#6b7280" : "#9ca3af", maxWidth: 280, margin: "0 auto", lineHeight: 1.7 } }, isOwner ? "Partage quelque chose avec ta communaut\xE9 !" : "Cet utilisateur n'a encore rien publi\xE9. Reviens plus tard !")
));
...
function ProfilePage({
  ...
  const requestCache = useRef(/* @__PURE__ */ new Map());
  ...
  const followUser = useCallback(async (uid) => {
    ...
    const { data } = await axios.post(
      ...
      {},
      { headers, withCredentials: true, timeout: 8e3 }
    );
    return data;
  ...
  const handlePostDeleted = useCallback(async (postId) => {
    ...
    showLocalToast("Post supprim\xE9");
      ...
      startTransition(() => {
        setProfilePosts((prev) => {
          ...
          const unique = merged.filter((p) => {
            ...
            seen.add(p._id);
            return true;
          }).map(normalizePost).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  ...
  const followStatus = profileUser && authUser && !isOwner ? (profileUser.followers || []).some(
  ...
  const handleFollowToggle = useCallback(async () => {
    ...
    const newFollowers = wasFollowing ? (profileUser.followers || []).filter(
      (u) => !isSameUser(typeof u === "object" ? u._id : u, authUserId)
    ...
    try {
      ...
      showLocalToast(wasFollowing ? "D\xE9sabonn\xE9 !" : "Abonn\xE9 !");
    } catch (err) {
      const rollback = wasFollowing ? [...profileUser.followers || [], authUserId] : (profileUser.followers || []).filter(
        (u) => !isSameUser(typeof u === "object" ? u._id : u, authUserId)
      );
      startTransition(() => setProfileUser((prev) => ({ ...prev, followers: rollback })));
      const status = err?.response?.status;
      if (status === 401 || status === 403)
        showLocalToast("Non autoris\xE9 \u2014 reconnecte-toi", "error");
      else if (status === 404)
        showLocalToast("Utilisateur introuvable", "error");
      else if (err.code === "ERR_NETWORK" || err.code === "ECONNABORTED")
        showLocalToast("Hors ligne \u2014 r\xE9essaie plus tard", "error");
      else
  ...
  useEffect(() => {
    ...
    if (navInstantUser && !initialUser && !silentRevalidatedRef.current) {
      ...
      if (targetUserId && isValidObjectId(targetUserId)) {
        ...
        if (navigator.onLine) {
          fetchUserById(targetUserId).then((fresh) => {
            if (fresh) {
    ...
    if (isMockId(targetUserId)) {
      ...
      if (result) {
        ...
        setIsLoadingUser(false);
        setUserNotFound(false);
        showLocalToast("Profil de d\xE9monstration", "info");
      }
      return;
    }
    ...
    (async () => {
      ...
      try {
        ...
        idbClearOtherKeys(`profilePosts_${targetUserId}`).then(() => profileDebug("mainEffect:idbClearOtherKeys:done", { targetUserId })).catch((err) => profileDebug("mainEffect:idbClearOtherKeys:error", { message: err?.message }));
        if (isOwner) {
          setProfileUser(authUser);
          setIsBot(false);
          setUserNotFound(false);
  ...
  const effectiveProfileUser = profileUser || (isValidObjectId(targetUserId) ? isOwner && authUser ? authUser : buildMinimalUser(targetUserId) : null);
  const stats = {
  ...
  const renderPostsFeed = () => {
    if (profilePosts.length === 0 && !isLoadingPosts) {
      return /* @__PURE__ */ React.createElement(EmptyPostsState, { isOwner, isDarkMode });
    }
    return /* @__PURE__ */ React.createElement("div", { className: "profile-feed-list" }, profilePosts.map((post, index) => {
      const isLast = index === profilePosts.length - 1;
      return /* @__PURE__ */ React.createElement("div", { key: post._id || index, ref: isLast ? lastPostRef : void 0 }, /* @__PURE__ */ React.createElement(
        PostCard,
        {
          post,
    ...
    }), isLoadingPosts && /* @__PURE__ */ React.createElement(LoadingSpinner, { darkMode: isDarkMode, text: "Chargement des publications..." }));
  };
  const renderContent = () => {
    if (selectedTab === "settings" && isOwner) {
      return /* @__PURE__ */ React.createElement("section", { className: "profile-section profile-panel" }, /* @__PURE__ */ React.createElement(SettingsSection, { user: effectiveProfileUser, showToast: showLocalToast }));
    }
    if (selectedTab === "feed") {
      return /* @__PURE__ */ React.createElement("section", { className: "profile-section profile-feed-section" }, isOwner && !isMockProfile && !isBot && /* @__PURE__ */ React.createElement(
        CreatePost,
        {
    ...
    return /* @__PURE__ */ React.createElement("section", { className: "profile-section" }, /* @__PURE__ */ React.createElement(
      ProfileMediaGrid,
      {
        posts: videoPosts,
        isDarkMode,
  ...
  if (authLoading || isLoadingUser && !effectiveProfileUser) {
    return /* @__PURE__ */ React.createElement("main", { style: { minHeight: "100vh", background: pageBg, padding: "32px 16px" } }, /* @__PURE__ */ React.createElement(LoadingSpinner, { darkMode: isDarkMode, text: "Chargement du profil..." }));
  }
  if (userNotFound && !effectiveProfileUser) {
    return /* @__PURE__ */ React.createElement("main", { style: { minHeight: "100vh", background: pageBg, padding: "32px 16px" } }, /* @__PURE__ */ React.createElement("div", { style: {
      maxWidth: 520,
      margin: "56px auto",
      padding: "34px 24px",
      textAlign: "center",
      ...
      fontFamily: "'Sora','DM Sans',sans-serif"
    } }, /* @__PURE__ */ React.createElement("h1", { style: { margin: "0 0 8px", fontSize: 22, fontWeight: 800 } }, "Profil introuvable"), /* @__PURE__ */ React.createElement("p", { style: { margin: 0, color: isDarkMode ? "#94a3b8" : "#64748b", fontSize: 14 } }, "Ce profil n'est pas disponible pour le moment.")), toast && /* @__PURE__ */ React.createElement(Toast, { message: toast.message, type: toast.type }));
  }
  return /* @__PURE__ */ React.createElement("main", { className: "profile-page-shell", style: { background: pageBg } }, /* @__PURE__ */ React.createElement("style", null, `
        .profile-page-shell {
          min-height: 100vh;
      ...
      `), /* @__PURE__ */ React.createElement("div", { className: "profile-page-frame" }, /* @__PURE__ */ React.createElement("div", { className: "profile-layout-card" }, /* @__PURE__ */ React.createElement(
    ProfileHeader,
    {
  ...
  ), !isOwner && /* @__PURE__ */ React.createElement("div", { style: {
    display: "grid",
  ...
  ), renderContent())), toast && /* @__PURE__ */ React.createElement(Toast, { message: toast.message, type: toast.type }));
}
export {
  clearProfilePostsCache,
  ProfilePage as default,
  readAllCachedProfilePosts
};
