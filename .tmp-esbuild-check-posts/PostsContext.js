...
var API_URL = import.meta.env.VITE_API_URL || (import.meta.env.PROD ? "https://chantilink-backend.onrender.com/api" : "http://localhost:5000/api");
var SESSION_LOAD_KEY = "posts_session_loaded_v1";
var PostsContext = createContext();
var usePosts = () => useContext(PostsContext);
var isValidObjectId = (id) => /^[a-f\d]{24}$/i.test(String(id || ""));
var BTP_KEYWORDS = [
  "chantier",
  "construction",
  "btp",
...
var normalizeLang = (lang = "fr") => {
  const code = String(lang || "fr").toLowerCase().split(/[-_]/)[0];
  return ["fr", "en", "ar"].includes(code) ? code : "fr";
};
var cleanBotText = (value = "") => String(value || "").replace(/https?:\/\/\S+/gi, "").replace(/\bt\.co\/\S+/gi, "").replace(/[@#][\p{L}\p{N}_-]+/gu, "").replace(/\s+/g, " ").trim();
var looksNoisyBotText = (value = "") => {
...
var isBotPost = (post) => !!(post?.isBot || post?.user?.isBot || post?.user?.isAutoCreated || post?._isBot);
var hasPostMedia = (post) => !!(post?.videoUrl || post?.embedUrl || post?.sourceUrl || post?.media?.length || post?.images?.length);
var isBtpPost = (post) => {
  const text = [
...
var _preloadInjected = /* @__PURE__ */ new Set();
